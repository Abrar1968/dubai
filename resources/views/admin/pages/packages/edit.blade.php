...
@section('header')
    <div class="flex items-center gap-4">
        ...
        <div>
            ...
            <p class="mt-1 text-sm text-slate-600">{{ $package->title }}</p>
        </div>
    </div>
@endsection

@section('content')
    <form action="{{ route('admin.hajj.packages.update', $package) }}" method="POST" enctype="multipart/form-data" x-data="packageForm()">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <x-admin.ui.card>
                    ...
                    <div class="space-y-4">
                        <x-admin.ui.input name="title" label="Title" :value="old('title', $package->title)" required placeholder="e.g., Premium Hajj Package 2025" />

                        <x-admin.ui.input name="slug" label="Slug" :value="old('slug', $package->slug)" placeholder="Leave empty to auto-generate" hint="URL-friendly identifier" />

                        <x-admin.ui.select name="type" label="Package Type" :options="collect($types)->mapWithKeys(fn($t) => [$t->value => ucfirst($t->value)])->toArray()" :value="old('type', $package->type->value)" required />

                        <x-admin.ui.textarea name="short_description" label="Short Description" :value="old('short_description', $package->short_description)" rows="2" maxlength="500" showCount placeholder="Brief description for package cards..." />

                        <x-admin.form.rich-editor name="description" label="Full Description" :value="old('description', $package->description)" />
                    </div>
                </x-admin.ui.card>
                ...
                <x-admin.ui.card>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <x-admin.ui.input type="number" name="price" label="Price (AED)" :value="old('price', $package->price)" required min="0" step="0.01" placeholder="0.00" />

                        <x-admin.ui.input type="number" name="discounted_price" label="Discounted Price (AED)" :value="old('discounted_price', $package->discounted_price)" min="0" step="0.01" placeholder="0.00" hint="Leave empty for no discount" />

                        <x-admin.ui.input type="number" name="max_capacity" label="Max Capacity" :value="old('max_capacity', $package->max_capacity)" min="1" placeholder="e.g., 50" />

                        <x-admin.ui.input type="number" name="available_slots" label="Available Slots" :value="old('available_slots', $package->available_slots)" min="0" placeholder="e.g., 25" />
                    </div>
                </x-admin.ui.card>
                ...
                <x-admin.ui.card>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <x-admin.ui.input type="number" name="duration_days" label="Duration (Days)" :value="old('duration_days', $package->duration_days)" required min="1" placeholder="e.g., 15" />

                        <x-admin.ui.input type="number" name="duration_nights" label="Duration (Nights)" :value="old('duration_nights', $package->duration_nights)" required min="0" placeholder="e.g., 14" />

                        <x-admin.ui.input name="departure_location" label="Departure Location" :value="old('departure_location', $package->departure_location)" placeholder="e.g., Dubai, UAE" />

                        <x-admin.form.date-picker name="departure_date" label="Departure Date" :value="old('departure_date', $package->departure_date?->format('Y-m-d'))" />

                        <x-admin.form.date-picker name="return_date" label="Return Date" :value="old('return_date', $package->return_date?->format('Y-m-d'))" />
                    </div>
                </x-admin.ui.card>
            ...
            <div class="space-y-6">
                ...
                <x-admin.ui.card>
                    ...
                    <div class="space-y-4">
                        <x-admin.ui.toggle name="is_active" label="Active" :checked="old('is_active', $package->is_active)" description="Make this package visible on the website" />

                        <x-admin.ui.toggle name="is_featured" label="Featured" :checked="old('is_featured', $package->is_featured)" description="Show on homepage featured section" />
                    </div>
    ...
    <script>
        function packageForm() {
            ...
            const oldInclusions = @json(old('inclusions'));
            const oldExclusions = @json(old('exclusions'));
            const oldItinerary = @json(old('itinerary'));
            const oldHotels = @json(old('hotel_details'));

            const packageFeatures = @json($package->features ?? []);
            const packageInclusions = @json($package->inclusions ?? []);
            const packageExclusions = @json($package->exclusions ?? []);
            const packageItinerary = @json($package->itinerary ?? []);
            const packageHotels = @json($package->hotel_details ?? []);

            return {
                features: (oldFeatures || packageFeatures).length
                    ? (oldFeatures || packageFeatures).map(v => ({ value: typeof v === 'object' ? v.value : v }))
                    : [{ value: '' }],
                inclusions: (oldInclusions || packageInclusions).length
                    ? (oldInclusions || packageInclusions).map(v => ({ value: typeof v === 'object' ? v.value : v }))
                    : [{ value: '' }],
                exclusions: (oldExclusions || packageExclusions).length
                    ? (oldExclusions || packageExclusions).map(v => ({ value: typeof v === 'object' ? v.value : v }))
                    : [{ value: '' }],
                itinerary: (oldItinerary || packageItinerary).length
                    ? (oldItinerary || packageItinerary)
                    : [{ title: '', description: '' }],
                hotels: oldHotels || packageHotels || [],
            }
        }
    </script>
@endsection
