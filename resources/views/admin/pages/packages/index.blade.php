...
@section('content')
        ...
        <div class="p-4 flex flex-wrap items-center gap-4">
            <div class="flex items-center gap-2">
                ...
                @foreach($types as $type)
                    <a href="{{ route('admin.hajj.packages.index', ['type' => $type->value]) }}"
        ...
        <x-admin.data.table :columns="[
            ...
            @foreach($packages as $package)
                <tr class="hover:bg-slate-50">
                    ...
                    <td class="px-6 py-4">
                        <div class="flex items-center gap-2">
                            <form action="{{ route('admin.hajj.packages.toggle-status', $package) }}" method="POST">
                                @csrf
                                @method('PATCH')
                    ...
                    <td class="px-6 py-4">
                        <div class="flex items-center justify-end gap-2">
                            <a href="{{ route('admin.hajj.packages.show', $package) }}" class="p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-lg" title="View">
                                <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z" />
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                </svg>
                            </a>
                            <a href="{{ route('admin.hajj.packages.edit', $package) }}" class="p-2 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg" title="Edit">
                                <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
    ...
    @else
        <x-admin.data.empty-state
            ...
            :actionHref="route('admin.hajj.packages.create')"
        />
    @endif
@endsection
