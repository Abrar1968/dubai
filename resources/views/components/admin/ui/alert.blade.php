...
>
    <div class="flex">
        ...
        <div class="@if($icon) ml-3 @endif flex-1">
            ...
            @endif
            @if($message)
                <p class="@if($title) mt-1 @endif text-sm {{ $config['text'] }}">{{ $message }}</p>
            @endif
            @if($slot->isNotEmpty())
                <div class="@if($title || $message) mt-2 @endif text-sm {{ $config['text'] }}">
                    {{ $slot }}
                </div>
            @endif
        </div>
        ...
        @if($dismissible)
            <div class="ml-auto pl-3">
                <button
                    type="button"
                    @click="show = false"
                    class="-mx-1.5 -my-1.5 inline-flex rounded-lg p-1.5 {{ $config['text'] }} hover:bg-opacity-20 focus:outline-none focus:ring-2 focus:ring-offset-2"
                >
