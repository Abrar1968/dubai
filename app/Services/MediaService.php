...
class MediaService
{
    ...
    public function uploadImage(
        ...
        $this->increaseMemoryForImage($file);
        ...
        try {
            ...
            // Free memory from content string
            unset($webpContent);

        } catch (\Throwable $e) {
            // On error, store original file as fallback
            $fallbackFilename = $this->generateFilename($file);
            Storage::disk($this->disk)->putFileAs($directory, $file, $fallbackFilename);
    ...
    /**
     * Increase PHP memory limit based on image dimensions.
     */
    protected function increaseMemoryForImage(UploadedFile $file): void
    {
        ...
        // Calculate required memory: width * height * bits/8 * channels * 2 (for processing overhead)
        // Add extra 50MB buffer for GD operations
        $requiredMemory = ($width * $height * ($bits / 8) * $channels * 2) + (50 * 1024 * 1024);

        $currentLimit = $this->getMemoryLimitInBytes();

        if ($requiredMemory > $currentLimit) {
            $newLimit = max($requiredMemory, 256 * 1024 * 1024); // At least 256MB
